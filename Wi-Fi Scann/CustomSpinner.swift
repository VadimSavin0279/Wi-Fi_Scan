...
final class CustomSpinnerSimple: UIView {
    ...
    override func layoutSubviews() {
        ...
        let rect = CGRect(x: bounds.width / 4 , y: -15, width: size, height: size)
        
        let path = UIBezierPath(arcCenter: CGPoint(x: rect.midX, y: rect.midY), radius: rect.height / 2, startAngle: 3 * CGFloat.pi / 2, endAngle: CGFloat.pi / 2, clockwise: true)
        shapeLayer.path = path.cgPath
        shapeLayer.fillColor = Asset.Colors.mainBlue.color.cgColor
    ...
    func startAnimation(delay: TimeInterval, replicates: Int) {
        ...
        let angle = CGFloat(2.0 * Double.pi) / CGFloat(replicates / 2)
        ...
        opacityAnimation.toValue = 0.2 // до какого значения
